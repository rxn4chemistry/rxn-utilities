...
tree_with_synthesis: Dict[str, Any] = {
    "id": 1,
    "configuration": {"action_sequence": [dummy_action]},
    "children": [
        {
            "id": 2,
            "configuration": {"action_sequence": [dummy_action]},
...
def test_has_children_with_synthesis():
    ...
    assert (
        has_children_with_synthesis(tree_with_synthesis["children"][0]["children"][0])
        is False
    )
    assert (
        has_children_with_synthesis(tree_with_synthesis["children"][0]["children"][1])
        is False
    )
    assert (
        has_children_with_synthesis(tree_with_synthesis["children"][0]["children"][2])
        is False
    )
    assert (
        has_children_with_synthesis(tree_with_synthesis["children"][1]["children"][0])
        is False
    )
    assert (
        has_children_with_synthesis(tree_with_synthesis["children"][1]["children"][1])
        is False
    )
...
def test_get_nodes():
    for identifier in range(1, 11):
        ...
        assert len(nodes) == 1
    nodes = get_nodes(
        tree, lambda node: node.get("id", "") == 1 or node.get("id", "") == 2
    )
