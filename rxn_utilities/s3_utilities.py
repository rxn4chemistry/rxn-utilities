...
def maybe_download_model_from_s3(model_uri: str) -> str:
    ...
    if model_uri.startswith("s3://"):
        try:
            ...
            connection = Minio(
                endpoint=host, access_key=access, secret_key=secret, secure=secure
            )
...
def post_process_model_information_for_name(
...
def post_process_model_information_for_forward(
    ...
    return {
        "name": model_information["name"],
        "has_support_for_light": model_information["forward_model"].get(
            "has_support_for_light", False
        ),
        "has_support_for_heat": model_information["forward_model"].get(
            "has_support_for_heat", False
        ),
    }
...
class RXNS3ModelClient(RXNS3Client):
    ...
    def process_model_metadata_entry(self, metadata_entry: str) -> Dict[str, Any]:
        ...
        try:
            response = self.client.get_object(
                bucket_name=self.bucket, object_name=metadata_entry
            )
    ...
    def ensure_model(self, path: str, model_type: str, model_tag: str) -> None:
        ...
        if not self.model_exists(path=path, model_tag=model_tag):
            ...
            logger.info(
                "Model {}/{} already exists in {}".format(model_type, model_tag, path)
            )
    ...
    def validate_tag(self, model_type: str, model_tag: str) -> None:
        ...
        if model_tag not in set(
            [
                ...
                for model_information in self.get_models_by_model_type(
                    model_type=model_type
                )
            ]
        ):
            error_message = (
                "Model tag {} is invalid for model type {} in bucket {}".format(
